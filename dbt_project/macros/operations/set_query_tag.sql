{% macro set_query_tag() -%}
  ...
  {% set dbt_is_cold_storage_refresh = 'false'%}

  {% if is_full_refresh_flagged() and is_cold_storage_flagged() %}
     {% set dbt_is_cold_storage_refresh = 'true' %}
  {% endif %}
  ...
  {% if dbt_model_name %}
    ...
    {% set new_query_tag = '{"dbt_environment_name": "%s", "dbt_job_id": "%s", "dbt_run_id": "%s", "dbt_run_reason": "%s", "dbt_project_name": "%s", "dbt_user_name": "%s", "dbt_model_name": "%s", "dbt_materialization_type": "%s", "dbt_incremental_full_refresh": "%s", "dbt_is_cold_storage_refresh": "%s"}'
