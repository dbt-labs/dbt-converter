...
source as (
    ...
    select * from {{ source('fivetran_googlesheets', 'global_training_feedback') }}
...
with_id as (
    ...
    select
        {{ dbt_utils.generate_surrogate_key(['response_row_id', 'submitted_by_org_name', 'submitted_at']) }} as unique_id,
        *
