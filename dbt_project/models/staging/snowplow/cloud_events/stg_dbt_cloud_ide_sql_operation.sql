...
joined as (
    ...
    select
        ...
        -- 30 April ’23, changed nomenclature from run_sql/compile_sql to preview/compile
        case
            when operation_start.operation_type in ('preview', 'run_sql')
                then 'show'
            when operation_start.operation_type = 'compile_sql'
                then 'compile'
            else operation_type          
        end as operation_type,

        case
            when operation_complete.operation_status = 'killed'
                then 'cancel'
            else operation_complete.operation_status
    ...
    left join operation_complete
        on operation_start.operation_id = operation_complete.operation_id
