...
exam_delivery_item_responses as (
    ...
    select * from {{ ref('stg_caveon__exam_delivery_item_responses') }}
    
),

exam_items as (

    select * from {{ ref('stg_caveon__exam_items') }}

),

users as (

    select * from {{ ref('stg_caveon__users') }}
...
final as (
    ...
    select
        ...
        exam_delivery_item_responses.delivery_item_response_id
