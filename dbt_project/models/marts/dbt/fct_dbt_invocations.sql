{{
    config(
        ...
        snowflake_warehouse = set_warehouse_config('large', 'xlarge'),
        cold_storage = true,
...
model_summary as (
    ...
    select * from {{ ref('dbt_model_summary') }}
...
rpc_tasks as (
    ...
    select * from {{ ref('stg_dbt_rpc_tasks') }}
...
xf as (
    ...
    select
        ...
        -- 2023-05-03: Adding a new ip_address (66.201.54.x) to this filter due to similar behavior, 
        -- though it may not be from the same customer -PB

        case
            when invocation_starts.ip_address in ('38.99.106.x','66.201.54.x')
                then coalesce(
        ...
        greatest(
            coalesce(
                invocation_starts._loaded_at,
                '01/01/1900'
            ),
            coalesce(
                invocation_ends._loaded_at,
                '01/01/1900'
            )
        ) as _loaded_at,
        coalesce(
...
id_hashing as (
    ...
    select
        *,
        {{ id_hashing(
            original_column_name = 'run_id_raw', 
            renamed_column_name = 'run_id'
        ) }},
        {{ id_hashing(
            original_column_name = 'develop_request_id_raw', 
