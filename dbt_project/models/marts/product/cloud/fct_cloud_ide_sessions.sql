...
invocations as (
    ...
    select * from {{ ref('stg_dbt_cloud_ide_invocation') }}

),

operations as (

    select * from {{ ref('stg_dbt_cloud_ide_sql_operation') }}

),

tab_views as (

    select * from {{ ref('stg_dbt_cloud_ide_tab_view') }}
...
ide_dag_sessions as (
    ...
    select * from {{ ref('int_ide_dag_sessions') }}
