...
dag_starts as (
    ...
    select * from {{ ref('stg_dbt_cloud__ide_dag_view_starts') }}

),

dag_requests as (

    select * from {{ ref('stg_dbt_cloud__knowledge_layer_requests') }}

),

dag_renders as (

    select * from {{ ref('stg_dbt_cloud__ide_dag_renders') }}

),

dag_interactions as (

    select * from {{ ref('stg_dbt_cloud__ide_node_interactions') }}
