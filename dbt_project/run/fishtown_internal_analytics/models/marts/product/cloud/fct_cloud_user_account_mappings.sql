        ...
        create or replace transient table analytics_dev.dbt_jstein.fct_cloud_user_account_mappings  as
        (
...
groups as (
    ...
    select * from analytics_dev.dbt_jstein.stg_cloud__groups
...
user_groups as (
    ...
    select * from analytics_dev.dbt_jstein.stg_cloud__user_groups
...
email_verified as (
    ...
    select * from analytics_dev.dbt_jstein.stg_cloud__email_verifications
...
permissions_joined as (
    ...
    select
        ...
        case
            when accounts.database_type = 'single-tenant'
                then 1
    ...
    left join cloud_sfdc_accounts using (account_id)
    left join user_license_groups using (user_license_id)
    left join verified_users using (user_id)
...
--joining the permissions_joined to users because some users might not have an account
    select
        ...
        permissions_joined.is_verified,
...
md5(cast(coalesce(cast(user_id as TEXT), '') || '-' || coalesce(cast(account_id as TEXT), '') as TEXT)) as user_account_id,
        *
