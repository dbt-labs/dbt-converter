        ...
        create or replace transient table analytics_dev.dbt_jstein.int_snowplow_sessions_tmp  as
        (

with

all_web_page_views as (
...
prep AS (
    ...
    select
        ...
        max(case when last_page_view_in_session = 1 then page_url else null end)
            as exit_page_url
        , max(case when last_page_view_in_session = 1 then database_source else null end)
            as database_source
...
sessions as (
    ...
    select
        ...
        a.device_type,
        a.device_is_mobile
        , a.database_source as first_database_source
        , b.database_source as last_database_source
