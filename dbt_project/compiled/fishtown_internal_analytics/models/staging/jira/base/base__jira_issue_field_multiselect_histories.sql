...
source as (
    ...
    select * from raw.fivetran_jira.issue_multiselect_history
...
renamed as (
    ...
    select
        md5(cast(coalesce(cast(_FIVETRAN_ID as TEXT), '') || '-' || coalesce(cast(time as TEXT), '') as TEXT)) as issue_field_multiselect_history_id,
