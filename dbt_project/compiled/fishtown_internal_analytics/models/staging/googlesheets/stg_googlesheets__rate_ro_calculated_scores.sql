-- Note:
-- This model can be replaced with a configuration sheet and the scores can be derived in dbt
-- but currently the logic is imbedded in the gsheet with array formulas
-- https://docs.google.com/spreadsheets/d/1ubFWDRPTrktNP5_emR-JP5K2t7TV8l8GPLTwn2p9FFM/edit#gid=48725690 

-- alternatively, we can keep using gsheets. Within gsheets we can change the weight of 
-- question responses and the weight of the multiple sections (Exit Criteria vs General Questions)

with source as (

    select * from raw.fivetran_googlesheets.rate_ro_calculated_scores
...
renamed as (
    ...
    select
        ...
        -- id and info
        md5(cast(coalesce(cast(account_id as TEXT), '') || '-' || coalesce(cast(timestamp as TEXT), '') as TEXT)) as rate_rapid_onboarding__survey_response_id,
        account_id,
        client_company_name,
