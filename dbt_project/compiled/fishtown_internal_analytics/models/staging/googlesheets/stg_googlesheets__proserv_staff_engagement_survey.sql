with source as (
    ...
    select * from raw.fivetran_googlesheets.ps_staff_engagement_survey
...
renamed as (
    ...
    select
        md5(cast(coalesce(cast(customer_account_id_from_salesforce_ as TEXT), '') || '-' || coalesce(cast(email_address as TEXT), '') || '-' || coalesce(cast(engagement_start_date as TEXT), '') as TEXT)) as survey_response_id,
